...
class UComicListViewController: BobBaseViewController {
    ...
    @objc private func loadData(more: Bool) {
        ...
        guard let key = argName else {
            return
        }
        
        guard let d = vm.getSectionDatas(with: key) else {
            return
        }
...
extension UComicListViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let vc = BobComicDetailViewController(comicid: model.comicId, comic: model)
