...
//        }
    ...
    var comicModel: ComicModel? {
        didSet {
            guard let d = comicModel else {
                return
            }
            bgView.kf.setImage(urlString: d.cover, placeholder: UIImage(named: "normal_placeholder_v"))
            coverView.kf.setImage(urlString: d.cover, placeholder: UIImage(named: "normal_placeholder_v"))
            nameLabel.text = d.name
            authorLabel.text = (d.description ?? "").count > 0 ? d.description : "孩子成长的好伙伴"
        }
    }
